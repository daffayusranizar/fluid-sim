...
* Note : The Guide line above predates the two-channel pressure revision (see the T-024 supplement and the Phase 1 invariants). Density here is **SpikyPow2 (main) + SpikyPow3 (near)**, not Poly6 — Poly6 was retired from density and now serves as the XSPH viscosity weight. Both pressure channels must be ported together: the near channel is a permanent outward push and the only thing that can balance it is a *negative* main pressure. The clamp is therefore ported as a **flag** (`clampPressurePositive`, default off), not as a hardcoded `max(0, p)`; hardcoding it would put the GPU into exactly the state `WarnIfClampConflictsWithNearChannel()` warns about. Every force/density term also carries `particleMass`, and density is mass-scaled.
...
* Note : Two corrections. (1) The Guide's "Viscosity Laplacian" is superseded (see the T-024 supplement): viscosity is **XSPH** — Poly6 as a non-negative weight on the velocity difference. A Laplacian can change sign and therefore add energy; a non-negative weight cannot, and it needs no square root. (2) The Spiky **gradient** functions have no `r >= h` early-out on the CPU — `SpikyPow2GradientMagnitude` returns a *negative* `(h - r)` past `h`. The CPU is only safe because the neighbour grid guarantees `r < h` before the call, so a brute-force GPU loop must make that test explicitly at every call site, including the boundary loop. Also: the mirrored boundary pressure is `max(0, p_i)` **unconditionally** — it is not governed by `clampPressurePositive` — and every force term carries `particleMass`. Integration (semi-implicit Euler plus the wall clamp) belongs to this task per its title and is ported as its own kernel; `ResetForces` is *not* ported, because the CPU force pass assigns rather than accumulates, making the reset redundant there too.
...
* Note : Implemented as decision #4(a). `SPHCompute` reduces `max‖v‖` and `max‖a‖` into a two-uint buffer using integer `InterlockedMax` on the float bit patterns — valid only because both are *magnitudes*, where the IEEE bit pattern is monotonic as an unsigned int. `SPHComputeSimulation` owns the frame loop and reproduces `SPHSolver.ComputeStableTimeStep`'s CFL form exactly, with the two inputs supplied by that reduction. Particle state ping-pongs between two buffers and does not return to the CPU during a step. The only readback left is the opt-in gizmo snapshot, which is the "read back for visualisation" allowance and is removed at T-032 by rendering straight from the buffer. `SPH2D.solveOnCpu` exists so the Burst solver does not advance the same particles twice.
...
| 3 | Boundary push on GPU (T-028) | pressure mirroring / penalty force / none | pressure mirroring | Matches the CPU exactly and the port is diffed against it at 9.0e-6 relative error, so the two cannot drift apart unnoticed. A penalty force would be a second, independently-tuned model of the same wall, and the two would disagree at exactly the moments that matter. "None" leaves only the position clamp, which is a penetration net rather than a wall model, and reintroduces wall welding. The mirrored pressure is `max(0, p_i)` regardless of `clampPressurePositive`, because copying a particle's own negative pressure would make the wall attractive. |
| 4 | Adaptive step under zero readback (T-029) | async readback / GPU-resident dt / fixed dt | GPU reduction + async readback, consumed a frame late | Keeps T-021's adaptive CFL step, which the Phase 1 invariants list as load-bearing; a fixed dt would have deleted it silently, which the task explicitly warns against. The one-frame lag costs nothing real, because the CPU already sized `dt` from the previous step's state. One request per frame, skipped while one is in flight, so readbacks cannot pile up. |
...
* Note : Both key schemes are implemented behind one `KeyForCell` function. The hash is exactly as the Guide specifies; the build **defaults to the direct index**. The T-023 note already observed that hashing exists for unbounded domains and measured slowest on a bounded one — and this domain is bounded, with `h` derived from spacing, which makes the uniform grid both smaller and collision-free. Measured at N=400: direct index → 441 buckets, 94 occupied, `occupied == distinctCells` (zero collisions), max occupancy 7. Hash with `tableSize = 2N+1` → 801 buckets, the same 94 distinct cells but only 89 occupied (5 collisions), max occupancy 11. A larger table does **not** reduce candidates per query: with cell size = `h` a cell holds ~4.8 particles in both modes, so a 3×3 scan sees the same ~43 candidates and the hash only adds false positives. Cell size is exactly `h` and the domain is the container plus `h` of margin, mirroring `SPH2D.RebuildNeighbors` so boundary particles land inside the grid instead of being clamped into edge cells. The per-particle cell coordinate is stored alongside the key because a hashed bucket cannot be inverted: a neighbour query has to reject candidates whose cell is not actually adjacent.
...
* Note : The instanced shader and the `DrawMeshInstancedIndirect` call already existed from the earlier visualisation work. What T-032 actually changes is the **source**: the renderer used to pack `SPH2D.Particles` and upload it with `SetData` every frame; it now binds the solver's own `ComputeBuffer` directly, so a GPU simulation renders with no readback and no upload. The CPU path is kept and is the automatic fallback (when no `SPHCompute` is initialised, `IsReady` is false and the renderer packs from `SPH2D` instead), so the two solvers can still be compared. The material buffer and the args buffer are only rebuilt when the source or the count changes, so a steady frame is one draw call with no buffer traffic. Verified by rendering to a `RenderTexture` and counting pixels: 400 particles produced 6411 lit pixels over 9.78% of the frame with `spreadX=0.26` / `spreadY=0.68` (spread over the cloud, not one blob), and the renderer allocated no CPU-side buffer at all. At 50k the whole render path costs 0.29 ms/frame including the camera render.
* Note : A `ComputeShader` asset is referenced in a scene as `fileID: 7200000`, not the `4800000` that `Shader` assets use. A wrong id deserialises to a silent `null` rather than an error, which is how this was found: the scene check reported `compute.shader=NULL` while every other reference resolved. Determined by letting Unity serialise the reference and reading the result back.

Task Code: T-033
* Quest : Improve fluid visualization with color mapping and screen-space smoothing.
* Guide : Color particles based on velocity magnitude or pressure in the fragment shader, using a multi-stop gradient (e.g. blue → cyan → green → yellow → orange → red) rather than a simple two-color blend, so speed changes are readable in fine detail. Render particle depth to an offscreen buffer, then apply a bilateral blur to smooth it while preserving edges. Reconstruct screen-space normals from the smoothed depth and use them for basic shading so the surface reads as one continuous blob instead of separate spheres.
* Done : Fluid shows smooth, multi-color gradation across the full speed range (not just two flat colors), and the surface looks continuous rather than like individual dots.
* Note : Three things found by checking this against the code before starting. (1) **The multi-stop gradient already exists.** `ParticleRenderer2D` builds a real `Gradient` per preset (four stops each: DeepWater, Molten, Neon, Monochrome), bakes it to a 128-sample bilinear texture and samples it by `speed / _VelocityMax`. So "not just two flat colors" is already true; what is left of that clause is stop count and perceptual spacing. (2) **Pressure colouring does not work on the GPU path.** The GPU physics writes `_Density`/`_Pressure` into separate buffers and nothing writes them into the `Particle` struct the renderer binds, so `p.pressure` is stale there. The CPU path does populate it (`RepackRenderBufferJob`), so pressure colour would work on one solver and silently not the other. Pick: bind the extra buffers to the render shader, or pack them. (3) **Every fragment currently has the same depth.** The vertex shader builds the quad at `float3(p.position + IN.vertex.xy * radius, 0)` -- `z` hardcoded to 0 -- and the material is `ZWrite Off` with alpha blending. A bilateral blur over a constant depth, and normals reconstructed from it, would produce a flat plane. The disc has to become a spherical impostor whose depth varies as `sqrt(1 - r²)` across the face, which is what makes the depth buffer meaningful.
* Note : **Part 2 status: implemented but NOT working; the feature is off by default.** The depth prepass is verified -- with the impostors rendered into an RFloat target under `BlendOp Min`, the background reads exactly 512 (the clear value), the surface reads ~9.9-10.0 view depth, and the field varies across the image. The bilateral blur and the composite are not correct. Evidence: after the horizontal blur pass, a background pixel that reads exactly 512.000 in the raw depth target reads 0.000, and the count of far pixels falls from 37624 to 9495 while the maximum stays 512 -- a partial write that the filter logic cannot produce (`centre >= _FarDepth*0.5` must return `_FarDepth` for a background pixel). `Material.GetTexture("_FluidDepth")` also reports unbound, though `SetFloat` on the same HLSL-only uniforms works, so the binding itself may be fine and the reporting unreliable. Next step is to isolate the RFloat render target + `Graphics.Blit` path on its own, since the bug is below the filter maths. Also still unverified: the hemisphere DEPTH may be inverted. The test only showed the depth varies; centre-vs-edge sampled between particles rather than across one, so whether the profile is convex (towards the camera) or concave is not yet established.
* Note : The Guide's second half is much larger than the earlier wording ("add a simple screen-space post-process"). Depth pass → bilateral blur → normal reconstruction → shading plus a full-screen composite needs either a URP `ScriptableRendererFeature` or self-managed render targets, and it changes the material from transparent-and-depthless to something that writes depth. Done in two commits: (a) spherical impostor with per-disc shading, verified by luminance varying across a disc; (b) depth target, bilateral blur, normals and the full-screen composite, verified by surface-continuity metrics (connected components, and perimeter per unit area) rather than lit-pixel counts, which cannot tell a surface from a cloud of discs.
...
## Phase 6: Interaction & macOS Export (T-038 to T-039)

All 2D phases are complete, so this phase makes the finished 2D simulation usable
before the 3D port starts. T-038 is deliberately the 2D half of T-037's scope,
done early: the 2D solver is validated and the 3D one is not, so interaction is
worth building where it can be trusted. T-037 keeps the 3D controls.

- [ ] T-038 — Mouse Interaction: tap, drag and pause
- [ ] T-039 — macOS Standalone Export

Task Code: T-038
* Quest : Make the fluid respond to the mouse: tap it to poke it, drag it to scoop it, freeze it with space.
* Guide : Read the cursor in a component that owns input and nothing else, convert screen space to world space through the camera, and publish a disc the solver treats as solid. Work out why a solid disc is safer than a force field for an SPH fluid, and what has to be capped so a fast drag cannot blow the solver up. Add a pause that does not lose state.
* Done : Pressing the left mouse button makes a disc solid under the cursor; dragging it scoops and carries the fluid; a tap pokes it; space freezes and resumes the simulation. Both the Burst solver and the GPU solver behave identically.
* Note : **A solid disc, not a force field.** A radial force injects velocity, which raises max||v||, which shrinks the adaptive CFL step, which at a fixed `maxSubSteps` cap makes the fluid run in slow motion rather than break. That failure is quiet and easy to misread as "the interaction feels weak". A position constraint plus a velocity resolve against the disc's own motion cannot do that, because the disc carries at most the velocity it was given. `SPHMath.ResolvePointerCollision` and the HLSL `ApplyPointer` are twins and have to stay in step; the only reason both solvers exist is that they can be compared.
* Note : **The disc is servoed to the cursor, not snapped.** Its velocity is measured from its own displacement, so `maxPointerSpeed` bounds what the fluid can ever be handed. Without the servo a flick across the window displaces a particle by many radii in one substep, and the projection stops being a constraint and becomes a teleport.
* Note : **Inflation is the tap strength control, and it belongs in the interactor rather than in the solver.** The first version appeared at full size and was too violent to use. What the pressure solve reacts to is penetration depth, and that is bounded during a drag for free: the disc is servo-capped, so it advances into the fluid by only `maxPointerSpeed * dt` per substep and no particle is ever deeper than that. A tap has no such bound, because the disc materialises around particles that are up to a full radius inside it and throws all of them out to the surface in a single substep -- a 0.62 unit displacement in 7e-4 s, with no velocity attached to it. Ramping the radius from zero bounds the same event by the growth rate instead, about 4 units/s against an implied 890, so the fluid sees a slow expansion rather than a void appearing. Nothing in `SPHMath` or the shader changed to achieve this: only `PointerState.radius` gained a ramp, because how the disc is presented is a separate question from what a solid disc is.
* Note : **Two details that only show up in use.** A click can begin and end inside one frame, so a quick tap would often do nothing without a minimum press latch. And the disc is clamped to stay a full radius clear of the walls, because a disc overlapping a wall pushes fluid into it every substep and fights the wall clamp, which reads as jitter rather than as a solid object. The cost is that the last radius of fluid against a wall cannot be reached. The latch turns out to limit how far a tap inflates as well as how long it survives, so `minimumPressDuration` doubles as a second tap strength control: at 0.12 against a 0.15 growth time a tap reaches about 80% of full radius.
* Note : `activeInputHandler` is `1` (Input System package only), so `UnityEngine.Input` throws at runtime in this project. The interactor is guarded with `#if ENABLE_INPUT_SYSTEM` / `ENABLE_LEGACY_INPUT_MANAGER` anyway, because that setting is one dropdown away from changing and an unguarded file would then fail to compile. Both branches were compiled against the 6000.6.0f1 assemblies with `dotnet build` on copies of the generated csproj files, which also caught two deprecations before they were committed: `FindFirstObjectByType` is obsolete in 6000.6, and a public `PointerState` field on a MonoBehaviour needs `[System.NonSerialized]` rather than `[HideInInspector]` (Unity's own serialisation analyser, UAC1001, found that one).
* Note : Verified by driving the real GPU pipeline from a throwaway editor script rather than by reading the code, then deleting the script. `SPHMath.ResolvePointerCollision` against five cases derived by hand (dead centre, outside, moving outward, restitution 1, inactive): 5/5. Then `SPHCompute.Initialize` on the modified shader for 13/13 kernels, which is also the only thing that actually compiles a compute shader -- Unity compiles them lazily, so an earlier successful build had proved nothing about the HLSL. Then a lattice at rest with a disc of radius 0.5 buried in it, stepped once with dt=1e-4: 169 particles inside beforehand, 0 penetrating afterwards, closest particle at 0.500000000. Then a particle isolated beyond all smoothing support, which is the only way to reach the kernel's zero-density early-out: it started exactly on the disc centre, so it also took the degenerate-normal branch, and landed at exactly 0.5.
* Note : Two things only that verification could have shown. (a) `AddComponent<SPH2D>()` in batch mode does not run `Awake`, so nothing spawns and the particle buffer reads back as every particle at the origin. The first attempt reported a closest particle of exactly 3.5000 -- the distance from the probe to the origin -- which was the entire clue. The harness builds its own lattice instead, which also made every number in the test visible. (b) A strict inside test counted 50 particles inside a disc that `minDistance` simultaneously reported as empty. Both were true: `position = centre + normal * radius` in 32-bit floats lands a few ulps under the radius, about 1e-7 at this scale. The assertion uses a 1e-4 margin, a thousand times the rounding and a hundred times below anything physical, and reports both counts so the two cannot be mistaken for each other. The disc surface is soft by 1e-7, which is not worth a fudge factor in the shader.
* Note : Still unverified as of this writing: how it feels. Whether 9 spacings is one scoopful, whether `friction` 0.35 scoops or merely shoves, and whether `maxPointerSpeed` 12 is both stable and responsive are all judgement calls that need hands on it. The pause is flag logic that no test covers.
* Note : `SPH2D.pointer` needed `[System.NonSerialized]`, not `[HideInInspector]`. HideInInspector only hides a field; Unity still serialises it and would write whatever the interactor last pushed into the scene file. Found by Unity's own serialisation analyser (UAC1001) on the first compile.

Task Code: T-039
* Quest : Export the simulation as a standalone macOS app that can be double-clicked.
* Guide : Build the player, and find out what an Editor-only success hides. Check every asset the runtime reaches for by name rather than by reference, and every setting that is only read at build time.
* Done : A `.app` bundle that launches windowed and is interactive, built from a script so the settings travel with the build.
* Note : **The build would have shipped a black window.** `ParticleRenderer2D` reached its shader with `Shader.Find("FluidSim/Particle2D")` and nothing in the project referenced that shader: no material asset, no serialised field, not in Always Included Shaders. Unity's own manual: a shader is included in a build only if something references it, and `Shader.Find` then resolves in the Editor and returns null in the player. The renderer would have logged an error and disabled itself. Fixed with a serialised `particleShader` field, and `FluidBuild.VerifySceneWiring` now refuses to build without it. The scene wiring was applied by running `FluidBuild.WireScene` in batch mode, which writes the real guid Unity generated for the new script rather than a hand-written one guessed into the YAML.
* Note : **`PlayerSettings.SetArchitecture` does not cover macOS.** Its documentation limits it to iOS, tvOS and visionOS, so using it here would have looked correct and silently left the default architecture in place. macOS is reached through `UnityEditor.OSXStandalone.UserBuildSettings.architecture`, which is what `BuildTarget.StandaloneOSX` itself points at. Verify the result on the artifact with `lipo -archs`, not on the setting.
* Note : Building from a script rather than the Build window, because the architecture, window mode and bundle identifier are project settings: a GUI build six months from now depends on what someone left in a dropdown. The script applies them, so the build carries its own configuration. `EditorApplication.Exit(1)` on failure matters in batch mode, where `-quit` otherwise exits 0 whatever happened.
* Note : Apple silicon only, Mono scripted, windowed at 1280x800. IL2CPP's advantages are smaller binaries and no JIT, which matter when other people run it; they cost minutes per build, which is paid on every iteration. Burst is unaffected either way, so switch to IL2CPP only for the build that leaves this machine.
* Note : Not yet run. `lipo -archs` on the built binary is the check that matters, not the setting, because the architecture is one of the things a build script can get wrong without saying so.
